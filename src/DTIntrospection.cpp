...
void DTIntrospection::initialize()
{
    ...
    if (_rootNode == nullptr) {
        throw std::runtime_error(
            "Root node is not set. You must first construct the decision tree before using introspection.");
    }
 ...
 * branch features and values, and samples at each node. It also maps samples to nodes and processes child nodes
 * recursively.
 *
...
void DTIntrospection::recursiveDescent(DecisionTreeNode* node)
{
    ...
    for (const auto &item : branchFeaturesAndValuesOrThresholds) {
        ...
        if (!samplesAtNode.has_value()) {
            ...
            std::set_intersection(samplesAtNode->begin(),
                                  samplesAtNode->end(),
                                  samplesForFeatureValueCombo.begin(),
                                  samplesForFeatureValueCombo.end(),
    ...
    if (samplesAtNode.has_value() && !samplesAtNode->empty()) {
        std::sort(samplesAtNode->begin(), samplesAtNode->end(), [&](int a, int b) -> bool { return a < b; });
    }
...
void DTIntrospection::recursiveDescentForShowingSamplesAtANode(DecisionTreeNode* node)
{
    ...
    if (_samplesAtNodesDict.find(nodeSerialNum) != _samplesAtNodesDict.end()) {
        if (_debug) {
            cout << "\nat node " << nodeSerialNum
                 << ": the branch features and values are: " << branchFeaturesAndValuesOrThresholds << endl;
        }
 ...
 * @param nodesAlreadyAccountedFor A vector of node serial numbers that have already been accounted for in the influence
 * path.
 * @param offset The indentation offset used for displaying the hierarchical influence path.
 */
void DTIntrospection::recursiveDescentForSampleToNodeInfluence(int nodeSerialNum,
                                                               vector<int> nodesAlreadyAccountedFor,
                                                               int offset)
{
    offset += 4;
    ...
    for (auto childSerialNum : childrenSerialNums) {
        if (std::find(nodesAlreadyAccountedFor.begin(), nodesAlreadyAccountedFor.end(), childSerialNum) ==
            nodesAlreadyAccountedFor.end()) {
...
void DTIntrospection::explainClassificationsAtMultipleNodesInteractively()
{
    ...
    if (_samplesAtNodesDict.empty()) {
        throw std::runtime_error("You called explainClassificationsAtMultipleNodesInteractively() without first "
                                 "initializing the DTIntrospection instance in your code. Aborting.");
    }

    cout << BOLD_BLUE + "\n\nIn order for the decision tree to introspect\n" + RESET;
    string msg = BOLD +
                 "  DO YOU ACCEPT the fact that, in general, a region of the feature space\n"
                 ...
                 RESET + BOLD + "\n    Enter " + BOLD_GREEN + "'y' for yes" + RESET + BOLD + " or any " + BOLD_RED +
                 "other character for no:  " + RESET;
    cout << msg;
    ...
    if (ans != "y" && ans != "yes") {
        throw std::runtime_error(
            BOLD_RED +
            "\n\n  Since you answered 'no' to a very real theoretical possibility, no explanations possible for the "
            "classification decisions in the decision tree. Aborting." +
            RESET);
    }
    ...
    while (true) {
        ...
        while (true) {
            cout << BOLD_BLUE + "\nEnter the integer ID of a node (1 to " +
                        std::to_string(_nodeSerialNumToNodeDict.size() - 1) + ") or type 'exit' to stop: " + RESET;
            std::getline(std::cin, ans);
            ans.erase(ans.find_last_not_of(" \n\r\t") + 1);
            ...
            try {
                ...
                cout << BOLD_RED + "\nWhat you entered does not look like an integer ID for a node. Aborting!" + RESET
                     << endl;
                return;
            }
            ...
            if (_samplesAtNodesDict.find(nodeId) != _samplesAtNodesDict.end()) {
                ...
                cout << BOLD_RED + "\nNode 0 is the root node. It has no samples. Try again or enter 'exit'." + RESET
                     << endl;
            }
            else {
                cout << BOLD_RED + "\nYour answer must be an integer ID of a node. Try again or enter 'exit'." + RESET
                     << endl;
            }
        }
 ...
 *
 * @throws std::runtime_error If the DTIntrospection instance is not initialized or if the user does not accept the
 * theoretical possibility that a region of the feature space may have non-zero probabilities even when there are no
 * training data points in that region.
 *
 ...
 * - Prompts the user to see the class probabilities using Bayes rule based on the last feature test on the branch
 * leading to the node.
...
void DTIntrospection::explainClassificationAtOneNode(int nodeId)
{
    ...
    if (_samplesAtNodesDict.empty()) {
        throw std::runtime_error("You called explainClassificationAtOneNode() without first initializing the "
                                 "DTIntrospection instance in your code. Aborting.");
    }
    ...
    if (!_awarenessRaisingMessageShown) {
        ...
        if (ans != "y" && ans != "yes") {
            throw std::runtime_error(BOLD_RED +
                                     "\n\n  Since you answered 'no' to a very real theoretical possibility, no "
                                     "explanations possible for the classification decision at node " +
                                     std::to_string(nodeId) + RESET);
        }
    ...
    if (!samplesAtNode.empty()) {
        ...
        msg2 =
            "\n\n    There are NO training data samples directly in the region of the feature space assigned to node " +
    ...
    msg2 += "\n";
    msg2 += BOLD_BLUE +
            "\n\nWould you like to see the probability associated with the last feature test on the branch leading to "
            "Node " +
            std::to_string(nodeId) + "?\n" + RESET;
    msg2 += BOLD + "\n    Enter " + BOLD_GREEN + "'y' for yes" + RESET + BOLD + " or any " + BOLD_RED +
            "other character for no:  " + RESET;
    ...
    string msg3 = BOLD_BLUE +
                  "\n\nUsing Bayes rule, would you like to see the class probabilities predicated on just the last "
                  "feature test on the branch leading to Node " +
                  std::to_string(nodeId) + "?\n" + RESET;
    msg3 += BOLD + "\n    Enter " + BOLD_GREEN + "'y' for yes" + RESET + BOLD + " or any " + BOLD_RED +
            "other character for no:  " + RESET;
    cout << msg3;
...
vector<int> DTIntrospection::getSamplesForFeatureValueCombo(string featureValueCombo)
{
    ...
    if (featureOpValue.op == "=") {
        for (const auto &samplePair : trainingDataDict) {
            ...
            if (std::find(featuresAndValues.begin(), featuresAndValues.end(), featureValueCombo) !=
                featuresAndValues.end()) {
