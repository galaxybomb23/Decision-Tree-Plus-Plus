...
TEST_F(UtilityTest, sampleIndex)
{
    ...
    _ASSERT_EQ(result.first, 1.0);
    _ASSERT_EQ(result.second, 0);
    ...
    _ASSERT_EQ(result2.first, 2);
    _ASSERT_EQ(result2.second, 6);
    ...
    _ASSERT_EQ(result3.first, "a");
    _ASSERT_EQ(result3.second, 4);
}
...
TEST_F(UtilityTest, ClosestSamplingPoint)
{
    ...
    _ASSERT_EQ(result2.value(), 4.0);

    // Test the ClosestSamplingPoint function on a vector of strings
    std::vector<std::string> vec3 = {"hello", "world", "this", "is", "a", "test"};
    std::optional<std::string> result3 = ClosestSamplingPoint(vec3, "is");
    _ASSERT_EQ(result3.value(), "is");
}
