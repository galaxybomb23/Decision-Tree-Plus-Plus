...
TEST_F(TrainingDataGeneratorNumericTest, TestReadParameterFileNumericAll)
{
    ...
    vector<string> featuresOrdered = tdgn.getFeaturesOrdered();
    ...
    map<string, pair<double, double>> featuresWithValueRange = tdgn.getFeaturesWithValueRange();
    ...
    map<string, map<string, vector<double>>> classesAndTheirParamValues = tdgn.getClassesAndTheirParamValues();
...
TEST_F(TrainingDataGeneratorNumericTest, TestGenerateMultivariateSamples)
{
    ...
    vector<double> mean = {50.0, 30.0};
    MatrixXd covMatrix(2, 2);
    covMatrix << 0.01, 0.0, 0.0, 0.01;
    int numSamples = 3000;
    vector<VectorXd> samples = tdgn.GenerateMultivariateSamples(mean, covMatrix, numSamples);
